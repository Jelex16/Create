...
class CardThing extends ConsumerStatefulWidget {
  const CardThing(
      {super.key, required this.model, this.onPressed, this.onPressedUpdate});
  final OfferCreateModel model;
  final void Function()? onPressed;
  final void Function()? onPressedUpdate;

  @override
  CardThingState createState() => CardThingState();
}

class CardThingState extends ConsumerState<CardThing> {
  @override
  void initState() {
    initData();
    super.initState();
    ...
    return Expanded(
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 5),
          Text('Estado: ${widget.model.estado?.value}',
              style: const TextStyle(fontSize: 15)),
          Text('Municipio: ${widget.model.municipio?.value}',
              style: const TextStyle(fontSize: 15)),
          Text('Colonia: ${widget.model.colonia?.value}',
              style: const TextStyle(fontSize: 15)),
