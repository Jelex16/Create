...
class CardAtendidos extends ConsumerStatefulWidget {
  const CardAtendidos(
      {super.key,
      required this.model,
  ...
  @override
  CardAtendidosState createState() => CardAtendidosState();
}

class CardAtendidosState extends ConsumerState<CardAtendidos> {
  @override
  ...
  Widget _data(UserModel2 user) {
    return Expanded(
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Text("${user.name!} \nde la inmobiliaria ${user.inmobiliario!} \nHizo Macht con tu requerimieno \nContactalo ", style: const TextStyle(fontSize: 12, fontWeight: FontWeight.bold)),
          const SizedBox(height: 15),
          Text("Ofrece ${widget.model.category!} a la ${widget.model.isSale! ? 'Venta' : 'Renta'}\na un precio de ${GeneralUtils.formatearPrecio(widget.model.precio!.value)}", style: const TextStyle(fontSize: 12, fontWeight: FontWeight.bold)),
          const SizedBox(height: 15),
          Text("En el Estado de ${widget.model.estado?.value} en el municipio de ${widget.model.municipio?.value} y en la colonia de ${widget.model.colonia?.value}", style: const TextStyle(fontSize: 12, fontWeight: FontWeight.bold)),
